...
public class TemplateMakerUtils {
     ...
     * @return
     */
    public static List<FilesDTO> removeGroupFilesFormRoot(
            List<FilesDTO> fileInfoList) {

        // 获取所有分组
        List<FilesDTO> groupFileInfoList = fileInfoList.stream()
                .filter(item -> CharSequenceUtil.isNotBlank(item.getGroupKey()))
                .collect(Collectors.toList());

        // 获取分组内的文件集合
        List<FilesDTO> groupInnerFileInfoList = groupFileInfoList.stream()
                .flatMap(item -> item.getFiles().stream())
                .collect(Collectors.toList());

        // 获取输入路径集合
        Set<String> inputPathSet = groupInnerFileInfoList.stream()
                .map(FilesDTO::getInputPath)
