...
import cn.xiao.springbootinit.model.dto.postfavour.PostFavourAddRequest;
import cn.xiao.springbootinit.model.dto.postfavour.PostFavourQueryRequest;
...
import cn.xiao.springbootinit.service.PostFavourService;
...
public class PostFavourController {
    ...
    public BaseResponse<Integer> doPostFavour(@RequestBody PostFavourAddRequest postFavourAddRequest,
        ...
        if (ObjectUtil.isEmpty(postFavourAddRequest) || postFavourAddRequest.getPostId() <= 0) {
    ...
    public BaseResponse<Page<PostVO>> listFavourPostByPage(@RequestBody PostFavourQueryRequest postFavourQueryRequest,
                                                           HttpServletRequest request) {
        if (ObjectUtil.isEmpty(postFavourQueryRequest)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        ...
        // 限制爬虫
        ThrowUtils.throwIf(size > 20 || ObjectUtil.isEmpty(userId), ErrorCode.PARAMS_ERROR);
        Page<Post> postPage = postFavourService.listFavourPostByPage(new Page<>(current, size),
